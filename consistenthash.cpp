...
class VirtualHost {
public:
  VirtualHost(string ip, PhscialHost *p) : ip_(ip), phscialHost_(p) {
...
public:
  ...
  string getHost(string clientip) const {
    ...
    //如果客户端的值太大，可能是环的另一端，这个时候需要取环开始的虚拟节点
    //映射从0开始遇见的第一个虚拟节点
...
void ShowConsistentHash(ConsistentHash &chash, string &msg);
...
int main() {
  ...
  PhscialHost host4("10.117.124.40", 150);
...
void ShowConsistentHash(ConsistentHash &chash, string &msg) {
  list<string> iplists{"192.168.1.123", "192.168.1.12", "192.168.1.13",
                       "192.168.1.23",  "192.168.1.54", "192.168.1.89",
                       "192.168.1.21",  "192.168.1.27", "192.168.1.49",
                       "192.168.1.145", "192.168.2.34", "192.168.6.78",
                       "192.168.2.90",  "192.168.4.5"};
